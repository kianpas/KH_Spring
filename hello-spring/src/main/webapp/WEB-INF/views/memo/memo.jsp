...
<div id="memo-container">
	...
	<table class="table">
	    ...
	    <tr>
	      <td>${memo.no}</td>
	      <td>${memo.memo}</td>
	      <td><fmt:formatDate value="${memo.regDate}" dateStyle="full"/></td>
	      <td><button type="button" class="btn btn-outline-danger" onclick="deleteMemo(${memo.no});">삭제</button></td>
		</tr>
		</c:forEach>
	</table>
</div>
<form action="${pageContext.request.contextPath}/memo/deleteMemo.do" method="post" name="deleteFrm">
	<input type="hidden" name="no" value=""/>
</form>
<script>
const deleteMemo=(no)=>{
	console.log(no)
	$frm = $(document.deleteFrm);
